...
public interface FeedsService {
	...
	/**
	 * Obtains the message with id from users posted messages
	 *
	 * @param user user feed being accessed (format user@domain)
	 * @param mid id of the message
	 *
	...
	@Path("/{" + USER + "}/own/{" + MID + "}")
	@Produces(MediaType.APPLICATION_JSON)
	Message getUserMessage(@PathParam(USER) String user, @PathParam(MID) long mid);

	/**
	 * Returns a list of all messages posted by a given user newer than time
	 *
	 * @param user user feed being accessed (format user@domain)
	 * @param time the oldest time of the messages to be returned
	 * @return	200 a list of messages, potentially empty;
	...
	@Path("/{" + USER +"}/own")
	@Produces(MediaType.APPLICATION_JSON)
	List<Message> getUserMessages(@PathParam(USER) String user, @QueryParam(TIME) long time);

	/**
	 * Create feed for user
	 *
	 * @param user user feed being created (format user@domain)
	 */
	@POST
	@Consumes(MediaType.APPLICATION_JSON)
	...
	/**
	 * Delete feed for user
	 *
	 * @param user user feed being deleted (format user@domain)
	 */
	@DELETE
	@Path("/{" + USER + "}")
	...
	/**
	 * Returns user feed last update time
	 *
	 * @param user user feed being accessed (format user@domain)
	 * @return	200 user feed last update time;
	 *  		404 if the user does not exist.
	 */
	@GET
	@Path("/{" + USER + "}/updatetime")
	long userLastUpdate(@PathParam(USER) String user);
}
